...
function checkFFmpegReady(options = {}) {
  const command = options.ffmpegCommand || 'ffmpeg';
  ...
  return {
    ready: !!(result.ok && output.toLowerCase().includes('ffmpeg')),
    path: command,
    version: versionLine || undefined,
    error: result.ok ? undefined : (result.error || 'ffmpeg -version failed')
  };
}
...
function discoverPython(customPythonPath, options = {}) {
  const candidates = uniqueList([
    customPythonPath,
    process.env.OPENSTEM_PYTHON_PATH,
    'python',
    'python3',
    'py'
  ]);
...
function checkBackendDetails(customPythonPath, options = {}) {
  ...
  if (!python.pythonFound) {
    ...
    const audioSeparatorImport = runCommand(python.pythonPath, ['-c', 'import audio_separator; print("audio_separator import OK")'], {
      ...options,
      timeout: 5000
    });
    ...
    if (audioSeparatorImport.ok) {
      ...
      if (!cli.ready) {
        result.blockers.push(makeBlocker('audio_separator_cli_missing', 'audio-separator CLI help could not be executed.', cli.error));
      }
    } else {
      result.blockers.push(makeBlocker('audio_separator_missing', 'Python package audio_separator could not be imported.', audioSeparatorImport.error));
    }
    ...
    if (torchProbe.ok) {
      ...
      result.blockers.push(makeBlocker('torch_missing', 'Python package torch could not be imported.', torchProbe.error));
    }
  }
  ...
  result.canRunAISeparation =
    result.pythonFound &&
    result.audioSeparatorInstalled &&
    result.audioSeparatorCliReady &&
    result.torchInstalled;
  result.canRunCpuAISeparation = result.canRunAISeparation && !!ffmpeg.ready;
...
function verifyOutputFolder(folderPath) {
  ...
  try {
    if (!fs.existsSync(folderPath)) {
      return { success: false, exists: false, writable: false, status: 'missing', error: 'Output folder does not exist.' };
    }
    const stats = fs.statSync(folderPath);
    if (!stats.isDirectory()) {
      return { success: false, exists: true, writable: false, status: 'invalid', error: 'Output path is not a directory.' };
    }
    ...
    return { success: false, exists: fs.existsSync(folderPath), writable: false, status: 'not_writable', error: err.message };
  }
}
...
function resolveAndVerifyModel(request, context = {}) {
  ...
  const payload = explicitModelPath
    ? { ...model, local_path: path.resolve(String(explicitModelPath)) }
    : model;
...
function validateProcessingRequest(request, context = {}) {
  ...
  const inputs = Array.isArray(request.inputs) ? request.inputs.filter(Boolean).map(input => path.resolve(String(input))) : [];
  if (inputs.length === 0) {
  ...
  let outputRoot = sameAsInput && inputs.length > 0 ? path.dirname(inputs[0]) : path.resolve(String(request.outputFolder || ''));
  if (!sameAsInput) {
  ...
  if (request.model && request.model.name) {
    ...
    if (!modelIntegrity.exists) {
      blockers.push(makeBlocker('model_missing', 'Selected model file is missing on disk.', modelIntegrity.localPath || modelIntegrity.error));
    } else if (modelIntegrity.status === 'hash_mismatch') {
      blockers.push(makeBlocker('model_hash_mismatch', 'Selected model hash does not match registry metadata.', modelIntegrity.localPath));
    } else if (modelIntegrity.status === 'size_mismatch') {
      blockers.push(makeBlocker('model_size_mismatch', 'Selected model size does not match registry metadata.', modelIntegrity.localPath));
    } else if (modelIntegrity.status === 'error' || modelIntegrity.ok === false) {
      blockers.push(makeBlocker('model_integrity_error', 'Selected model could not be verified.', modelIntegrity.error));
    } else if (!modelProofEligibility.proofEligible) {
      const blockerId = modelProofEligibility.reason === 'hash_missing'
        ? 'model_hash_missing'
        : `model_${modelProofEligibility.reason}`;
      blockers.push(makeBlocker(blockerId, modelProofEligibility.displayMessage, modelIntegrity.localPath || modelIntegrity.error));
    }
  }

  const ffmpeg = context.ffmpeg || checkFFmpegReady(context);
  if (!ffmpeg.ready) {
    blockers.push(makeBlocker('ffmpeg_missing', 'FFmpeg is missing or ffmpeg -version failed.', ffmpeg.error));
  }

  const backendDetails = context.backendDetails || checkBackendDetails(request.customPythonPath, { ...context, ffmpeg });
  ...
  if (backendDetails.pythonFound && !backendDetails.audioSeparatorInstalled) {
    blockers.push(makeBlocker('audio_separator_missing', 'audio-separator import failed in the selected Python environment.'));
  }
  if (backendDetails.pythonFound && backendDetails.audioSeparatorInstalled && !backendDetails.audioSeparatorCliReady) {
    blockers.push(makeBlocker('audio_separator_cli_missing', 'audio-separator CLI could not be inspected with --help.'));
  }
  ...
  if (requestedDevice !== 'cpu') {
    warnings.push(makeBlocker('device_forced_cpu', `Requested device "${requestedDevice}" ignored for this task; CPU mode is forced.`));
  }
...
function scanVerifiedOutputs(outputDir, beforeSnapshot = new Map()) {
...
function createProofResult({ exitCode, outputFiles = [], error, status }) {
  const verifiedOutputs = outputFiles.filter(file => file && file.verified && file.sizeBytes > 0);
  ...
  return {
    ...
    error: proofPass ? undefined : (error || 'AI CPU separation did not produce verified non-empty output stems.'),
    status
  };
}
...
function createCpuProcessEnv(extraEnv = {}) {
  return {
    ...process.env,
...
function resolveTargetOutputDir(request, normalized, inputTrack) {
  let targetOutDir = normalized.outputMode === 'same_as_input'
    ? path.dirname(inputTrack)
...
async function runCpuAiSeparation(request, options = {}) {
  ...
  onProgress({ type: 'process', status: 'running', log: '[backend] Launching audio-separator subprocess in CPU mode.' });
  ...
  try {
    for (const inputTrack of normalized.inputs) {
      if (isCancellationRequested()) {
        const cancelled = createProofResult({ exitCode: -1, outputFiles: allOutputFiles, error: 'Processing cancelled.', status: 'cancelled' });
      ...
      onLog(`[backend-cli] ${invocation.command} ${invocation.args.map(arg => String(arg).includes(' ') ? `"${arg}"` : arg).join(' ')}`);
      if (!invocation.deviceFlagApplied) {
        onLog('[backend-cli] Installed audio-separator CLI help does not expose --device; CPU mode is enforced with process environment GPU masking.');
      }

      const child = spawnProcess(invocation.command, invocation.args, {
        cwd: targetOutDir,
        env: createCpuProcessEnv(options.env),
        windowsHide: true
      });
      ...
      if (childResult.cancelled) {
        const cancelled = createProofResult({ exitCode: lastExitCode, outputFiles: allOutputFiles, error: 'Processing cancelled.', status: 'cancelled' });
        onProgress({ type: 'process', status: 'cancelled', error: cancelled.error });
        return cancelled;
      }
      ...
      for (const output of newOutputs) {
        onLog(`[backend] Output verification: ${path.basename(output.path)} (${output.sizeBytes} bytes, verified=${output.verified})`);
      }
