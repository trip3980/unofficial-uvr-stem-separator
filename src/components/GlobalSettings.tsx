      ...
      <div className="p-4 sm:p-6 rounded-2xl bg-[#090b14]/90 border border-green-500/15 shadow-2xl relative overflow-hidden w-full min-w-0 box-border">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-green-500/10 pb-5 mb-5 w-full min-w-0">
          ...
          <div className="flex flex-wrap gap-2 w-full sm:w-auto">
            <HelpToggle sectionId="global_settings" label="Show Help" className="flex-1 sm:flex-none justify-center px-3 py-1.5" />
            <button 
        ...
        <div className="space-y-4 w-full min-w-0">
          ...
          <div className="border border-white/5 rounded-xl overflow-hidden bg-black/20">
            ...
            >
              <div className="flex items-center gap-2.5 min-w-0">
                ...
                <span className="text-xs md:text-sm font-bold text-slate-100 uppercase tracking-wider font-mono">1. General Process Defaults</span>
                <span className="px-1.5 py-0.5 rounded text-[8px] font-mono leading-none font-bold bg-emerald-950/40 text-emerald-400 border border-emerald-500/20">Wired Defaults</span>
              </div>
              {expanded[0] ? <ChevronUp className="w-4 h-4 text-slate-400" /> : <ChevronDown className="w-4 h-4 text-slate-400" />}
            </button>
            {expanded[0] && (
              <div className="p-4 border-t border-white/5 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <label className="flex flex-col gap-1.5">
                    <span className="text-[10px] font-mono uppercase text-emerald-400/80 font-bold">Default Process Method</span>
                    <select 
                  ...
                  <div className="space-y-2 border-t border-white/5 pt-2">
                    <span className="text-[10px] font-mono uppercase text-[#9e76fa] font-bold block">Input Behavior</span>
                    <label className="flex items-center gap-3 p-2 rounded bg-black/30 border border-purple-500/10 cursor-not-allowed opacity-60">
                      <input type="checkbox" disabled checked={false} className="w-4 h-4 rounded border-purple-500/20 bg-black text-purple-500" />
                      <div className="text-2xs font-mono text-purple-400">
                        <span>Remember last input folder</span>
                        <span className="block text-[9px] text-slate-500 italic">Legacy UVR5 Reference / Not wired</span>
                      </div>
                    </label>

                    <label className="flex items-center gap-3 p-2 rounded bg-black/40 border border-green-500/15 cursor-default">
                      <input type="checkbox" checked={true} readOnly className="w-4 h-4 rounded border-emerald-500 bg-black text-emerald-500 accent-emerald-500" />
                      <div className="text-2xs font-mono text-emerald-300">
                        <span>Allow Multiple Input Files</span>
                        <span className="block text-[9px] text-[#55f28c]/60 italic font-sans font-bold">Active default • Fully wired</span>
                      </div>
                    </label>
                    ...
                    <label className="flex items-center gap-3 p-2 rounded bg-black/40 border border-green-500/15 cursor-default">
                      ...
                      <div className="text-2xs font-mono text-emerald-300">
                        ...
                        <span className="block text-[9px] text-[#55f28c]/60 italic font-sans font-bold">Active block logic • Wired</span>
                      </div>
                    </label>
                  </div>
                </div>
                ...
                <div className="space-y-3.5">
                  <label className="flex flex-col gap-1.5">
                    <span className="text-[10px] font-mono uppercase text-emerald-400/80 font-bold">Default Save Format</span>
                    <select 
                      value={state.outputFormat}
                      onChange={(e) => setState(prev => ({ ...prev, outputFormat: e.target.value as OutputFormat }))}
                      className="w-full bg-black/65 border border-green-500/25 rounded-lg px-3 py-2 text-xs text-green-300 focus:outline-none focus:border-green-400 font-mono"
                    >
                  ...
                  <div className="space-y-2 border-t border-white/5 pt-2">
                    <span className="text-[10px] font-mono uppercase text-emerald-400/80 font-bold block">Output Behavior</span>
                    ...
                    <label className="flex items-center gap-3 p-2 rounded bg-black/30 border border-purple-500/10 cursor-not-allowed opacity-60">
                      <input type="checkbox" disabled checked={false} className="w-4 h-4 rounded border-purple-500/20" />
                      <div className="text-2xs font-mono text-purple-400">
                    ...
                    <div className="p-2 border-t border-white/5 space-y-1.5 opacity-60">
                      <span className="text-[9px] font-mono uppercase text-purple-400 font-bold block">Stop/Cancel Subspace</span>
                      <div className="grid grid-cols-3 gap-1">
                        {["Confirm Stop", "Kill Process Tree", "Partial Kept"].map((n, i) => (
                          <div key={i} className="bg-black/40 border border-purple-500/15 py-1.5 px-2 rounded text-[8px] font-mono text-[#9b76fa] text-center select-none" title="UVR5 process management adapter is delegated. Not wired to direct JS execution.">
                            {n}
                          </div>
                        ))}
                      </div>
          ...
          <div className="border border-white/5 rounded-xl overflow-hidden bg-black/20">
            ...
            >
              <div className="flex items-center gap-2.5 min-w-0">
                ...
                <span className="text-xs md:text-sm font-bold text-slate-100 uppercase tracking-wider font-mono">2. Default VR Architecture Settings</span>
                <span className="px-1.5 py-0.5 rounded text-[8px] font-mono leading-none font-bold bg-indigo-950/40 text-indigo-400 border border-indigo-500/20">Parameters</span>
              </div>
              ...
              <div className="p-4 border-t border-white/5 space-y-4 text-xs font-mono">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-3">
                    <label className="flex flex-col gap-1.5">
                      <span className="text-[10px] font-mono uppercase text-emerald-400/80 font-bold">Default VR Mode Model Selection</span>
                      <select 
                    ...
                    <label className="flex flex-col gap-1.5">
                      <span className="text-[10px] font-mono uppercase text-emerald-300 font-bold">Window (Segment) Size</span>
                      <select 
                    ...
                    <div className="p-2 bg-black/30 border border-purple-500/10 rounded opacity-60">
                      ...
                      <input type="range" disabled min="0" max="100" value="10" className="w-full h-1 bg-purple-950/20 outline-none" />
                      <div className="text-[9px] text-slate-500 italic mt-1 leading-normal font-sans">
                        <b>Aggression:</b> Higher values increase vocal removal strength but may damage instrumentals. <span className="font-mono text-purple-400">(Legacy UVR5 Reference / Not wired)</span>
                      </div>
                    </div>
                  </div>
                    ...
                    <label className="flex items-center gap-3 p-2 rounded bg-black/30 border border-purple-500/10 cursor-not-allowed opacity-60">
                      ...
                      <div className="text-2xs font-mono text-purple-400">
                        ...
                        <span className="block text-[9px] text-slate-500 italic font-sans leading-normal">
                          <b>TTA:</b> Runs additional predictions to improve quality but increases processing time. <span className="font-mono text-purple-400">(Legacy UVR5 Reference)</span>
                        </span>
                      </div>
                    </label>
          ...
          <div className="border border-white/5 rounded-xl overflow-hidden bg-black/20">
            ...
            >
              <div className="flex items-center gap-2.5 min-w-0">
                ...
                <span className="text-xs md:text-sm font-bold text-slate-100 uppercase tracking-wider font-mono">3. Default MDX-Net Settings</span>
                <span className="px-1.5 py-0.5 rounded text-[8px] font-mono leading-none font-bold bg-fuchsia-950/40 text-fuchsia-400 border border-fuchsia-500/20">Chunks</span>
              </div>
            ...
            {expanded[2] && (
              <div className="p-4 border-t border-white/5 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-3">
                    <label className="flex flex-col gap-1.5">
                      <span className="text-[10px] font-mono uppercase text-emerald-400/80 font-bold">Default MDX Model Selection</span>
                      <select 
                        value={state.selectedModelId}
                        onChange={(e) => setState(prev => ({ ...prev, selectedModelId: e.target.value }))}
                        className="w-full bg-black/65 border border-green-500/25 rounded-lg px-3 py-2 text-xs text-green-300 font-mono focus:outline-none"
                    ...
                    <label className="flex flex-col gap-1.5">
                      <span className="text-[10px] font-mono uppercase text-emerald-300 font-bold">Active Chunk Size</span>
                      <select 
                        value={state.chunks} 
                      ...
                      <span className="text-[10px] text-slate-400 italic font-sans leading-relaxed">
                        <b>Chunks:</b> Smaller chunks use less RAM/VRAM but may increase processing time. Larger chunks may process faster but use more memory.
                      </span>
                    </label>
                  </div>
                  ...
                  <div className="space-y-3">
                    <label className="flex flex-col gap-1.5">
                      <span className="text-[10px] font-mono uppercase text-emerald-350 font-bold">Noise Reduction Strength</span>
                      <select 
                      ...
                      <span className="text-[10px] text-slate-400 italic font-sans leading-relaxed">
                        <b>Noise Reduction:</b> Higher values reduce model noise more aggressively but can affect output quality.
                      </span>
                    </label>
                    ...
                    <div className="grid grid-cols-2 gap-2 pt-1">
                      <label className="flex flex-col gap-1.5 justify-between p-2 rounded bg-black/40 border border-emerald-500/15">
                        ...
                        <input 
                          type="checkbox"
                          checked={state.saveNoiseyOutput}
                          onChange={(e) => setState(prev => ({ ...prev, saveNoiseyOutput: e.target.checked }))}
                          className="w-4 h-4 rounded border-emerald-500 text-emerald-500 accent-emerald-500 mt-1"
                        />
                      </label>

                      <div className="flex flex-col justify-between p-2 rounded bg-black/30 border border-purple-500/10 opacity-60">
                        <span className="text-[9px] font-mono text-purple-400 font-bold uppercase">Demucs secondary pass</span>
          ...
          <div className="border border-white/5 rounded-xl overflow-hidden bg-black/20">
            ...
            >
              <div className="flex items-center gap-2.5 min-w-0">
                <Sliders className="w-4 h-4 text-[#bf82e2] shrink-0" />
                <span className="text-xs md:text-sm font-bold text-slate-100 uppercase tracking-wider font-mono">4. Default Demucs Settings</span>
                <span className="px-1.5 py-0.5 rounded text-[8px] font-mono leading-none font-bold bg-[#38104c]/40 text-[#ce91fa] border border-[#a24ce0]/20">Stems</span>
              </div>
            ...
            {expanded[3] && (
              <div className="p-4 border-t border-white/5 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-3">
                    <label className="flex flex-col gap-1.5">
                      <span className="text-[10px] font-mono uppercase text-emerald-400/80 font-bold">Default Demucs Model</span>
                      <select 
                        value={state.selectedModelId}
                        onChange={(e) => setState(prev => ({ ...prev, selectedModelId: e.target.value }))}
                        className="w-full bg-black/65 border border-green-500/25 rounded-lg px-3 py-2 text-xs text-green-300 font-mono focus:outline-none"
                  ...
                  <div className="space-y-2 opacity-65">
                    <span className="text-[10px] font-mono uppercase text-purple-400 font-bold block">Legacy Demucs Pass-Throughs</span>
                    ...
                    <div className="grid grid-cols-2 gap-2 text-[10px]">
                      <div className="bg-black/35 border border-purple-500/10 p-2 rounded font-mono text-purple-400">
                        ...
                        <span className="block text-[8px] text-slate-500 italic mt-0.5">UVR5 Reference / Not wired</span>
                      </div>
                      <div className="bg-black/35 border border-purple-500/10 p-2 rounded font-mono text-purple-400">
          ...
          <div className="border border-white/5 rounded-xl overflow-hidden bg-black/20">
            ...
            >
              <div className="flex items-center gap-2.5 min-w-0">
                ...
                <span className="text-xs md:text-sm font-bold text-slate-100 uppercase tracking-wider font-mono">5. Default Ensemble Settings</span>
                <span className="px-1.5 py-0.5 rounded text-[8px] font-mono leading-none font-bold bg-[#43230b]/40 text-amber-400 border border-amber-500/20">Fusions</span>
              </div>
            ...
            {expanded[4] && (
              <div className="p-4 border-t border-white/5 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-3">
                    <label className="flex flex-col gap-1.5">
                      <span className="text-[10px] font-mono uppercase text-emerald-400/80 font-bold">Default Ensemble Preset</span>
                      <select 
                  ...
                  <div className="space-y-2 opacity-65">
                    <span className="text-[10px] font-mono uppercase text-purple-400 font-bold block">Ensemble Advanced Alignments</span>
                    <div className="p-3 bg-black/40 border border-purple-500/1 y-2 rounded space-y-1 text-2xs text-[#a277fa] font-mono">
                      <div>• Algorithm focus: Instrumental [Min Spec] / Vocals [Max Spec] <span className="block text-[8px] text-slate-500 italic mt-0.5">UVR5 Reference / Not wired</span></div>
                      <div>• Minimum track requirements: Enforced strictly in pipeline state (Requires &ge; 2 tracks)</div>
                      <div>• Custom templates: Locked to official model manifest <span className="block text-[8px] text-slate-500 italic mt-0.5">Planned / Not active</span></div>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="border border-white/5 rounded-xl overflow-hidden bg-black/20">
            ...
            >
              <div className="flex items-center gap-2.5 min-w-0">
                ...
                <span className="text-xs md:text-sm font-bold text-slate-100 uppercase tracking-wider font-mono">6. Model Test Mode Defaults</span>
                <span className="px-1.5 py-0.5 rounded text-[8px] font-mono leading-none font-bold bg-[#142d1e]/40 text-[#49dd86] border border-[#1b5833]/20">Active Evaluation</span>
              </div>
              {expanded[5] ? <ChevronUp className="w-4 h-4 text-slate-400" /> : <ChevronDown className="w-4 h-4 text-slate-400" />}
            </button>
            {expanded[5] && (
              <div className="p-4 border-t border-white/5 space-y-4">
                <div className="flex items-center justify-between border-b border-white/5 pb-1 gap-2 flex-wrap">
                  ...
                  <div className="space-y-3.5">
                    <label className="flex items-center gap-3 p-3 rounded bg-black/40 border border-green-500/15 justify-between">
                      <div className="text-2xs font-mono text-emerald-300">
                        ...
                        <span className="block text-[9px] text-[#55f28c]/60 italic font-sans mt-0.5">Prevents identical files from overwriting during comparison</span>
                      </div>
                      <input 
                        type="checkbox"
                  ...
                  <div className="space-y-3">
                    <label className="flex items-center gap-3 p-3 rounded bg-black/40 border border-green-500/15 justify-between">
                      <div className="text-2xs font-mono text-emerald-300">
                        ...
                        <span className="block text-[9px] text-[#55f28c]/60 italic font-sans mt-0.5">Places each stem structure in isolated subdirectory per test</span>
                      </div>
                      <input 
                        type="checkbox"
          ...
          <div className="border border-white/5 rounded-xl overflow-hidden bg-black/20">
            ...
            >
              <div className="flex items-center gap-2.5 min-w-0">
                ...
                <span className="text-xs md:text-sm font-bold text-slate-100 uppercase tracking-wider font-mono">7. Output Stem Defaults</span>
                <span className="px-1.5 py-0.5 rounded text-[8px] font-mono leading-none font-bold bg-green-950/40 text-green-400 border border-green-500/20">Stems Only</span>
              </div>
              {expanded[6] ? <ChevronUp className="w-4 h-4 text-slate-400" /> : <ChevronDown className="w-4 h-4 text-slate-400" />}
            </button>
            {expanded[6] && (
              <div className="p-4 border-t border-white/5 space-y-4">
                <div className="flex items-center justify-between border-b border-white/5 pb-1 gap-2 flex-wrap">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2.5 opacity-65">
                    <span className="text-[10px] font-mono uppercase text-purple-400 font-bold block">Legacy Squelch / Silent Gates</span>
                    <div className="p-2.5 bg-black/35 border border-purple-500/10 rounded space-y-1 text-2xs text-[#a277fa] font-mono">
                      <div>• Output collision: Ask User / Never Overwrite / Replace <span className="block text-[8px] text-[indigo-300]/50 italic">UVR5 Reference / Not wired</span></div>
                      <div>• Silent Squelching: Mute empty stems <span className="block text-[8px] text-[indigo-300]/50 italic">UVR5 Reference / Not wired</span></div>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="border border-white/5 rounded-xl overflow-hidden bg-black/20">
            ...
            >
              <div className="flex items-center gap-2.5 min-w-0">
                ...
                <span className="text-xs md:text-sm font-bold text-slate-100 uppercase tracking-wider font-mono">8. Hardware and Performance Defaults</span>
                <span className="px-1.5 py-0.5 rounded text-[8px] font-mono leading-none font-bold bg-[#153123]/40 text-[#4ddd91] border border-[#1b5835]/20">Execution</span>
              </div>
              {expanded[7] ? <ChevronUp className="w-4 h-4 text-slate-400" /> : <ChevronDown className="w-4 h-4 text-slate-400" />}
            </button>
            {expanded[7] && (
              <div className="p-4 border-t border-white/5 space-y-4 text-xs font-mono">
                <div className="flex items-center justify-between border-b border-white/5 pb-1 gap-2 flex-wrap">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-3">
                    <label className="flex flex-col gap-1.5">
                      <span className="text-[10px] uppercase text-emerald-400 font-bold">Default Compute Device Selection</span>
                      <select 
                  ...
                  <div className="space-y-2 opacity-65">
                    <span className="text-[10px] uppercase text-purple-400 font-bold block">Engine Resource Throttling</span>
                    <div className="p-3 bg-black/40 border border-purple-500/10 rounded space-y-1 text-2xs text-[#9d74f8]">
                      <div>• VRAM Safe Mode: Disables aggressive weight pre-allocation <span className="block text-[8px] text-slate-500 italic">UVR5 Reference / Not wired</span></div>
                      <div>• Process priority: Normal (Delegated) <span className="block text-[8px] text-slate-500 italic">UVR5 Reference / Not wired</span></div>
          ...
          <div className="border border-white/5 rounded-xl overflow-hidden bg-black/20">
            ...
            >
              <div className="flex items-center gap-2.5 min-w-0">
                ...
                <span className="text-xs md:text-sm font-bold text-slate-100 uppercase tracking-wider font-mono">9. Paths and Runtime Defaults</span>
                <span className="px-1.5 py-0.5 rounded text-[8px] font-mono leading-none font-bold bg-[#142d20]/40 text-[#4add8e] border border-[#235837]/20">Paths configuration</span>
              </div>
              {expanded[8] ? <ChevronUp className="w-4 h-4 text-slate-400" /> : <ChevronDown className="w-4 h-4 text-slate-400" />}
            </button>
            {expanded[8] && (
              <div className="p-4 border-t border-white/5 space-y-4 text-xs font-mono">
                <div className="flex items-center justify-between border-b border-white/5 pb-1 gap-2 flex-wrap">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2.5">
                    <span className="text-[10px] uppercase text-purple-400 font-bold block">Environmental Decoupled Paths</span>
                    ...
                    <div className="p-2.5 bg-black/35 border border-purple-500/10 rounded space-y-2 text-2xs text-[#9c72f7]">
                      <div>
                        ...
                        <span className="font-bold underline text-slate-400">FFmpeg DSP Fallback</span>
                        <div className="text-[9px] text-slate-500 italic leading-normal font-sans pt-0.5">
                          <b>FFmpeg DSP Fallback:</b> Non-AI static DSP filtering / Not AI model separation.
                        </div>
                      </div>
          ...
          <div className="border border-white/5 rounded-xl overflow-hidden bg-black/20">
            ...
            >
              <div className="flex items-center gap-2.5 min-w-0">
                ...
                <span className="text-xs md:text-sm font-bold text-slate-300 uppercase tracking-wider font-mono">10. Logging and Troubleshooting</span>
                <span className="px-1.5 py-0.5 rounded text-[8px] font-mono leading-none font-bold bg-[#291740]/40 text-purple-400 border border-purple-500/20">Reference</span>
              </div>
              {expanded[9] ? <ChevronUp className="w-4 h-4 text-slate-400" /> : <ChevronDown className="w-4 h-4 text-slate-400" />}
            </button>
            {expanded[9] && (
              <div className="p-4 border-t border-white/5 text-xs text-purple-400 font-mono space-y-3">
                ...
                />
                <span className="text-[10px] uppercase font-bold text-purple-400 block pb-1 border-b border-purple-500/10 opacity-65 cursor-not-allowed">Legacy Diagnostics Suite [Not active]</span>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <div>• Active Logger State: Disabled <span className="block text-[8px] text-slate-500 italic">UVR5 Reference / Not wired</span></div>
                    <div>• Clean Logs Schedule: 7 Days <span className="block text-[8px] text-slate-500 italic">UVR5 Reference / Not wired</span></div>
                  </div>
                  <div className="space-y-1.5">
                    <button disabled className="w-full py-1.5 px-3 bg-[#1d0e33] border border-purple-500/20 rounded text-center text-purple-400 hover:text-purple-300 text-[10px] cursor-not-allowed select-none">
          ...
          <div className="border border-white/5 rounded-xl overflow-hidden bg-black/20">
            ...
            >
              <div className="flex items-center gap-2.5 min-w-0">
                ...
                <span className="text-xs md:text-sm font-bold text-slate-100 uppercase tracking-wider font-mono">11. Reset and Safety Controls</span>
                <span className="px-1.5 py-0.5 rounded text-[8px] font-mono leading-none font-bold bg-[#431418]/40 text-rose-400 border border-rose-500/20">Wired Actions</span>
              </div>
              {expanded[10] ? <ChevronUp className="w-4 h-4 text-slate-400" /> : <ChevronDown className="w-4 h-4 text-slate-400" />}
            </button>
            {expanded[10] && (
              <div className="p-4 border-t border-white/5 space-y-4 text-xs font-mono">
                <div className="flex items-center justify-between border-b border-white/5 pb-1 gap-2 flex-wrap">
                ...
                <p className="text-[11px] font-sans text-slate-400 leading-relaxed max-w-2xl whitespace-normal break-words">
                  Destructive operations require explicit local confirmations. Executing standard flushes cleans isolated temporary files and clears corrupt download caches safely.
                </p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
          <div className="p-6 rounded-2xl bg-[#0b0f19] border border-slate-800 shadow-2xl max-w-md w-full relative space-y-4 text-slate-200 font-sans leading-relaxed">
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 mt-1.5 whitespace-normal break-words leading-relaxed">
                  ...
                  {modalType === "import_export" && "Review or import global preferences JSON configuration map below. Copy and paste direct JSON properties safely."}
                </p>
              </div>
            </div>
            ...
            <div className="flex justify-end gap-2 pt-2 text-xs font-mono">
              ...
              {modalType === "reset" && (
                <button onClick={handleHardReset} className="px-4 py-2 rounded bg-rose-950/80 hover:bg-rose-900 border border-rose-500 text-rose-200 font-bold">
                  Reset Global Preferences
                </button>
              )}
              {modalType === "restore_safe" && (
                <button onClick={handleRestoreSafeDefaults} className="px-4 py-2 rounded bg-emerald-950/80 hover:bg-emerald-900 border border-emerald-500 text-emerald-200 font-bold">
                  Restore Safe Defaults
                </button>
              )}
              ...
              {modalType === "import_export" && (
                <button onClick={handleImportSettings} className="px-4 py-2 rounded bg-emerald-950/80 hover:bg-emerald-900 border border-emerald-500 text-emerald-200 font-bold">
                  Import Map
                </button>
              )}
            </div>
