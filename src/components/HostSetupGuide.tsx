...
const WINDOWS_STEPS: StepItem[] = [
  {
    ...
    purpose:
      "Perform production build check (npm run build) and TypeScript check (npx tsc --noEmit) before local runtime configuration.",
...
npx tsc --noEmit`,
  },
  {
    ...
    purpose:
      "Create isolated local Python sandbox to avoid package conflicts. Python 3.10 is preferred for structural stability.",
...
python -m venv .venv-ai`,
  },
  {
    ...
    purpose:
      "Activate the sandboxed environment context in Windows PowerShell. Note: ExecutionPolicy configuration changes apply only to the current PowerShell process session and do not permanently change global system policy.",
...
pip install "audio-separator"`,
  },
  {
    ...
    purpose:
      "Optional CUDA path / only if NVIDIA driver and compatible PyTorch CUDA wheel are present. Install PyTorch CUDA wheels compatible with your NVIDIA driver and CUDA runtime. Replaces/upgrades the PyTorch wheel set.",
...
pip install "audio-separator[gpu]" --upgrade`,
  },
  {
    ...
    purpose:
      "Check PyTorch runtime, GPU availability, and FFmpeg path. Note: FFmpeg may be available through system PATH or through the app’s configured FFmpeg path in Global Settings. If using a custom FFmpeg path, verify it through Global Settings diagnostics.",
    command: `nvidia-smi
ffmpeg -version
...
.\\.venv-ai\\Scripts\\python.exe -c "import torch; print('Torch:', torch.__version__); print('CUDA Available:', torch.cuda.is_available())"`,
  },
  {
    ...
    purpose:
      "Conduct E2E separation checks. CPU proof is the simplest local AI E2E proof path. CUDA is optional and not locally proven until this CUDA E2E proof passes with real non-empty separated stems.",
...
npx tsc --noEmit`,
  ...
  },
  {
    ...
    purpose:
      "Verify macOS local environment readiness. PyTorch may expose Apple MPS when the installed PyTorch build, macOS version, and hardware support it.",
...
python -c "import torch; print('Torch:', torch.__version__); print('MPS Available:', torch.backends.mps.is_available() if hasattr(torch.backends, 'mps') else False)"`,
  },
  {
    ...
    name: "7. Run AI E2E Proof",
    purpose:
      "Execute local E2E proof script. CPU check is recommended first. Apple MPS is available only on supported macOS/PyTorch/Apple Silicon setups and is not locally proven until a real E2E run passes.",
    command: `# Define proof paths first:
...
pip install "audio-separator"`,
  },
  {
    ...
    name: "5b. Install (Optional CUDA Path)",
    purpose:
      "Optional CUDA path / only for NVIDIA systems. Install PyTorch CUDA wheels compatible with your NVIDIA driver and CUDA runtime. Replaces/upgrades the PyTorch wheel set.",
    command: `pip install torch torchvision torchaudio --index-url https://download.pytorch.org/whl/cu121
pip install "audio-separator[gpu]" --upgrade`,
  },
  {
    ...
    name: "6. Verify Drivers & Paths",
    purpose:
      "Confirm Linux environment state, PyTorch CUDA GPU indicators, and FFmpeg capability. Note: FFmpeg may be available through system PATH or through the app’s configured FFmpeg path in Global Settings. If using a custom FFmpeg path, verify it through Global Settings diagnostics.",
    command: `nvidia-smi
ffmpeg -version
python -c "import torch; print(torch.cuda.is_available()); print(torch.version.cuda); print(torch.cuda.get_device_name(0) if torch.cuda.is_available() else 'None')"`,
  },
  {
    ...
    name: "7. Run AI E2E Proof",
    purpose:
      "Execute local E2E proof script. CPU check is recommended first. CUDA is optional and not locally proven until this CUDA E2E proof passes with real non-empty separated stems.",
    command: `# Define proof paths first:
PYTHON_EXE_PATH="./.venv-ai/bin/python"
MODEL_FILE_PATH="<MODEL_FILE_PATH>"
EXPECTED_SHA256="<EXPECTED_SHA256>"
...
export const HostSetupGuide: React.FC = () => {
  ...
  const activeSteps = osTab === "win" ? WINDOWS_STEPS : osTab === "mac" ? MACOS_STEPS : LINUX_STEPS;
    ...
    <div className="p-4 bg-[#0a0e17]/80 border border-slate-800/80 rounded-xl space-y-4 font-sans text-xs text-slate-300">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-white/5 pb-2.5">
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-slate-100 uppercase tracking-wide font-mono text-[11px]">
            Terminal Host Setup Environment Guide
          </span>
        </div>
      ...
      <div className="bg-emerald-950/20 px-3 py-2.5 border border-emerald-500/10 rounded-xl text-[11px] text-emerald-300 leading-relaxed space-y-1.5 font-sans">
        ...
        <p className="text-slate-400 text-[10.5px]">
          By default, OpenStem operates locally by running local Python backend processes for AI model execution. This
          guide helps prepare a local environment. Setup alone does not prove the AI backend. Copy and paste
          instructions into your command terminal interface manually. Note: No automated scripts or hidden packages will
          alter your system global configurations unless run manually on your end.
        </p>
      </div>
      ...
      <div className="bg-slate-950/45 px-3 py-2.5 border border-white/5 rounded-xl text-[10.5px] text-slate-400 leading-relaxed space-y-2 font-sans">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          <div>
            ...
            <p className="mt-1">
              Checks packaged app resources, helper scripts, and Electron bridge files without claiming backend
              readiness.
            </p>
          </div>
          <button
        ...
        {runtimeDiagnosticError && (
          <div className="text-amber-300 font-mono text-[9.5px]">{runtimeDiagnosticError}</div>
        )}
            ...
            {runtimeDiagnostic.missingRequiredFiles.length > 0 && (
              <ul className="list-disc pl-4 text-[9px] space-y-0.5">
                {runtimeDiagnostic.missingRequiredFiles.map((missingPath) => (
                  <li key={missingPath} className="break-all">
                    {missingPath}
                  </li>
                ))}
              </ul>
      ...
      <div className="bg-amber-950/20 px-3 py-2.5 border border-amber-500/15 rounded-xl text-[10.5px] text-slate-400 leading-relaxed space-y-2 font-sans">
        ...
        <p>
          Electron packages the OpenStem desktop shell and native bridge. Python, FFmpeg, PyTorch, audio-separator,
          CUDA, model files, and verified AI proof remain runtime-managed unless a separate bundling and license
          strategy is created.
        </p>
        <ul className="list-disc pl-4 space-y-1">
          <li>
            Local CPU backend detection has passed for a project-local <code>.venv-openstem</code> runtime with Python
            3.11.9, audio-separator, and PyTorch 2.12.1+cpu. FFmpeg is checked separately and may remain missing until
            PATH or a selected executable is configured. CUDA remains false.
          </li>
          <li>
          ...
          <li>
            The local <code>UVR-MDX-NET-Inst_HQ_1.onnx</code> candidate remains blocked because its SHA-256 does not
            match the registry checksum.
          </li>
          <li>
            A model source returning HTTP 401 is classified as <code>auth_required</code>, not <code>broken_link</code>{" "}
            or no-internet. Hugging Face 401 sources may require login, token access, approved gated-model access, or
            corrected metadata, and remain blocked until legitimate access plus SHA-256 verification are available.
          </li>
          <li>
            No HTTP response is classified separately as <code>network_unavailable</code>, <code>dns_failed</code>, or{" "}
            <code>timeout</code>.
          </li>
          <li>
            Manual import is valid only when the user supplies model source metadata and expected SHA-256 values that
            can be verified against the local file. Candidate sources remain unverified until license, expected hash,
            and local hash all match.
          </li>
          <li>
            Browser Preview remains not runnable for native downloads, model verification, backend execution, or output
            proof.
          </li>
        </ul>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3.5 pt-1">
        ...
        <div className="md:col-span-2 space-y-3 bg-[#080b13]/60 p-3.5 rounded-xl border border-white/5 font-sans">
          <div className="border-b border-white/5 pb-1.5">
            <h6 className="font-bold text-slate-200 text-[11px] uppercase tracking-wide font-mono text-emerald-400">
              {currentStep.name}
            </h6>
          ...
          <div className="relative group rounded-lg border border-black bg-black/85 font-mono text-[10px] overflow-hidden">
            ...
            <div className="absolute top-2.5 right-2.5">
              <AccessibleTooltipWrapper
                content="Click to copy this terminal command block to clipboard safely."
                position="top"
              >
                <button
                  type="button"
                  ...
                  aria-label={
                    copiedText === "failed"
                      ...
                      : copiedText === currentStep.id
                        ...
                        : `Copy command for ${currentStep.name}`
                  }
          ...
          <div className="space-y-1.5 text-[9.5px] text-slate-400 font-sans border-t border-white/5 pt-2">
            ...
            <ul className="list-disc pl-3.5 space-y-1 text-slate-400">
              <li>
                Review placeholders before running. These commands are copied for manual terminal use only. The app does
                not execute them from this panel.
              </li>
              <li>
                Package versions may need adjustment depending on Python version, OS, CUDA version, GPU driver, and
                audio-separator release.
              </li>
              <li>
                Installing packages depending on environment configuration can sometimes fail. CPU checks remain the
                highest compatibility baseline.
              </li>
            </ul>
          </div>

          <div className="bg-[#0b101c]/40 p-2.5 rounded-lg border border-slate-800/80 text-[10px] text-slate-400 italic">
            <span className="font-semibold text-slate-300 font-sans not-italic block mb-0.5">
              💡 Execution Guidance Note:
            </span>
            Make sure to apply these steps directly into your system-level command line interface where Python is fully
            initialized.
          </div>
        </div>
      </div>
      ...
      <div className="border-t border-white/5 pt-4 space-y-4">
        <div className="p-3 bg-slate-950/60 rounded-xl border border-white/5 space-y-2.5">
          ...
          <p className="text-[10px] text-slate-400 font-sans leading-relaxed">
            <strong>Proof Boundary Statement:</strong> This guide outlines reference setup pathways. Simply running
            these commands manually does not verify host AI compliance within this applet context. AI proof strictly
            requires executing <code>test-ai-e2e.cjs</code> with <code>--expected-sha256</code>, a successful{" "}
            <strong>PASS / exit code 0</strong>, and files greater than 0 bytes written to disk. The{" "}
            <strong>Beta Candidate release state still requires final release checklist review</strong>.
          </p>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-2 text-[9px] font-mono">
            {[
              "Golden Proof Model: Missing",
              "Golden Proof Model: Selected / Not verified",
              "Golden Proof Model: Hash mismatch",
              "Golden Proof Model: Hash verified",
              "Golden Proof Model: Proof ready",
              "Golden Proof Model: Proof passed",
            ].map((label) => (
              <div key={label} className="rounded border border-white/5 bg-black/25 px-2 py-1 text-slate-400">
                {label}
              </div>
            ))}
          </div>
            ...
            <div className="space-y-1.5">
              <div>
                ...
                <p className="text-slate-400 text-[9px]">
                  Only valid if actual physical AI-separated output stems (.wav, .mp3, etc.) exist inside your
                  designated folder, and are greater than 0 bytes. FFmpeg fallback does NOT count as local AI separation
                  proof.
                </p>
              </div>
              <div>
                ...
                <p className="text-slate-400 text-[9px]">
                  The setup script was loaded but failed during execution or modeling. Verify configuration or review
                  full terminal stacktrace output. Keep release label as Alpha.
                </p>
              </div>
            </div>
            <div className="space-y-1.5">
              <div>
                ...
                <p className="text-slate-400 text-[9px]">
                  E2E test failed due to a missing/invalid requirement: bad Python path, missing audio-separator
                  package, model file not found at path, or bad input audio files.
                </p>
              </div>
              <div>
                <strong className="text-slate-400">● FFmpeg Path fallback:</strong>
                <p className="text-slate-400 text-[9px]">
                  FFmpeg must be configured through system PATH or a selected executable in Global Settings / Classic
                  Console / Batch Encoder. Fallback mode is a secondary mechanism and cannot serve as AI proof
                  validation.
                </p>
              </div>
            </div>
        ...
        <div className="p-3 bg-slate-950/45 rounded-xl border border-white/5 space-y-2">
          ...
          <p className="text-[9.5px] text-slate-400 font-sans leading-relaxed">
            When reporting a successful local setup run, you may manually fill and submit this standardized E2E gate
            validation template. Note that Beta Candidate still requires final release checklist review.
          </p>
